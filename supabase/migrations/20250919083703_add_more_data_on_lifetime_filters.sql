...
DECLARE
    ...
    min_ad_date timestamp with time zone;
    max_ad_date timestamp with time zone;
BEGIN
    ...
    -- For lifetime queries without other filters, determine actual date range for efficiency
    IF NOT has_date_filter AND 
       ...
       (angel_param IS NULL OR angel_param = 'all') THEN
        ...
        -- Get the actual date range of ads for this brand to avoid scanning unnecessary data
        SELECT 
            MIN(a.source_created_time),
            MAX(a.source_created_time)
        INTO min_ad_date, max_ad_date
        FROM ads a
        ...
        -- If no ads found, return empty result
        IF min_ad_date IS NULL THEN
            SELECT json_build_object(
                'daily_sentiment_counts', '[]'::json,
                'total_sentiment_counts', json_build_object(
                    'positive', 0,
                    'negative', 0,
                    'neutral', 0
                ),
        ...
        -- Use the actual date range for efficient filtering
        start_date_ts := min_ad_date;
        end_date_ts := max_ad_date;
    END IF;

    -- Main query logic (same for all cases now, including lifetime)
    ...
    -- For lifetime view, aggregate by month; for others, by day
    time_aggregated_sentiments AS (
        SELECT
            CASE 
                WHEN NOT has_date_filter THEN TO_CHAR(DATE_TRUNC('month', created_time), 'YYYY-MM-DD')::TEXT
                ELSE DATE(created_time)::TEXT
            END AS created_date,
            SUM(CASE WHEN lower(trim(sentiment::text)) = 'positive' THEN 1 ELSE 0 END) AS positive_count,
            SUM(CASE WHEN lower(trim(sentiment::text)) = 'negative' THEN 1 ELSE 0 END) AS negative_count,
            SUM(CASE WHEN lower(trim(sentiment::text)) NOT IN ('positive','negative') THEN 1 ELSE 0 END) AS neutral_count
        FROM filtered_comments
        WHERE created_time IS NOT NULL
        GROUP BY 1
        ORDER BY 1
        LIMIT CASE WHEN NOT has_date_filter THEN 120 ELSE 365 END
    ),
    ...
    SELECT json_build_object(
        'daily_sentiment_counts', COALESCE((SELECT json_agg(t) FROM time_aggregated_sentiments t), '[]'::json),
        ...
        'theme_distribution', COALESCE((SELECT json_agg(t) FROM theme_counts t), '[]'::json),
        ...
        'ads', COALESCE((
            ...
            FROM filtered_ads fa
            LIMIT CASE WHEN NOT has_date_filter THEN 5000 ELSE 1000 END
        ), '[]'::json),
        'comments', COALESCE((
            SELECT json_agg(
                json_build_object(
            ...
            LIMIT CASE WHEN NOT has_date_filter THEN 10000 ELSE 5000 END
        ), '[]'::json),
        'brand_status', (
            SELECT json_build_object(
                'is_ad_analyzing', COALESCE(b.is_ad_analyzing, false),
