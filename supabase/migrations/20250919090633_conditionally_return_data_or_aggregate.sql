-- Add a parameter to conditionally return full data or just aggregates
-- When return_full_data is false (dashboard), only return aggregates/counts
-- When return_full_data is true (brands page), return full data arrays
...
CREATE OR REPLACE FUNCTION get_dashboard_data(
    ...
    return_full_data BOOLEAN DEFAULT FALSE  -- New parameter: false for dashboard, true for brands page
)
RETURNS JSON
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    -- For lifetime queries without other filters, use optimized approach
    IF NOT has_date_filter AND 
       ...
       (angel_param IS NULL OR angel_param = 'all') THEN
        ...
        SELECT json_build_object(
            ...
            -- Only return actual data arrays if return_full_data is true
            'campaigns', '[]'::json,
            'ad_sets', '[]'::json,
            'ads', '[]'::json,
            'comments', '[]'::json,
    ...
    SELECT json_build_object(
        ...
        -- Conditionally return full data arrays based on return_full_data parameter
        'campaigns', CASE 
            WHEN return_full_data THEN
                COALESCE((
                    SELECT json_agg(
