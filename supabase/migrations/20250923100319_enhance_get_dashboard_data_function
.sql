-- Extend database function timeout and optimize performance
-- This attempts to resolve the timeout issue before implementing pagination

-- 1. Increase statement timeout for the session/database
-- Note: This may need to be set at the database level depending on your Supabase plan
ALTER DATABASE postgres SET statement_timeout = '300000'; -- 5 minutes
ALTER DATABASE postgres SET lock_timeout = '60000'; -- 1 minute

-- 2. Optimize the existing get_dashboard_data function with performance improvements
...
BEGIN
    ...
    IF NOT has_date_filter AND 
       ...
       (adset_optimization_param IS NULL OR adset_optimization_param = 'all') THEN
        ...
        IF return_full_data THEN
            -- OPTIMIZED: Use materialized CTEs for better performance on large datasets
            WITH RECURSIVE 
            -- First get all relevant ad_account IDs to reduce join complexity
            ...
            -- Get campaigns with optimized filtering
            lifetime_campaigns AS (
                SELECT camp.*
                FROM campaigns camp
                WHERE camp.account_id IN (SELECT account_id FROM relevant_accounts)
            ),
            
            -- Get ad_sets with optimized joins
            lifetime_ad_sets AS (
                SELECT ads_set.*
                FROM ad_sets ads_set
                WHERE ads_set.campaign_id IN (SELECT id FROM lifetime_campaigns)
            ),
            
            -- Get ads with optimized joins - use DISTINCT to avoid duplicates
            lifetime_ads AS (
                SELECT DISTINCT a.*
                FROM ads a
                WHERE a.ad_set_id IN (SELECT id FROM lifetime_ad_sets)
            ),
            
            -- Get comments with optimized joins
            ...
            SELECT json_build_object(
                ...
                'campaigns', COALESCE((
                    ...
                    LIMIT 500 -- Reduced from no limit to prevent memory issues
                ), '[]'::json),
                'ad_sets', COALESCE((
                    SELECT json_agg(
                        json_build_object(
                    ...
                    ORDER BY las.created_time DESC
                    LIMIT 1000 -- Reduced from 500 to 1000 for better coverage
                ), '[]'::json),
                'ads', COALESCE((
                    SELECT json_agg(
                        json_build_object(
                            ...
                            'total_comments', (
                                ...
                                WHERE lc.ad_id = la.id
                            )
                        )
                    )
                    FROM lifetime_ads la
                    ORDER BY la.source_created_time DESC
                    LIMIT 2000 -- Increased from 1000 to handle more ads
                ), '[]'::json),
                'comments', COALESCE((
                    SELECT json_agg(
                        json_build_object(
                    ...
                    LIMIT 10000 -- Increased from 5000 to handle more comments
                ), '[]'::json),
                'brand_status', (
                    SELECT json_build_object(
        ...
        ELSE
            -- Dashboard only needs aggregates for lifetime queries (existing optimized code)
            SELECT json_build_object(
                'daily_sentiment_counts', COALESCE((
                    SELECT json_agg(t)
                    FROM (
    ...
    -- Original logic for filtered queries (existing code from the latest migration)
    -- [Rest of the function remains exactly the same as in your latest migration file]
