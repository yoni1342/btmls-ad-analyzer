-- PERFORMANCE FIX: Optimize get_dashboard_data to avoid query timeouts
-- Simplify joins and use more efficient query patterns
...
BEGIN
    ...
    SELECT json_build_object(
        'daily_sentiment_counts', (
            SELECT json_agg(
                json_build_object(
                    'created_date', DATE(fc.created_time)::TEXT,
                    'positive_count', COUNT(*) FILTER (WHERE lower(trim(fc.sentiment::text)) = 'positive'),
                    'negative_count', COUNT(*) FILTER (WHERE lower(trim(fc.sentiment::text)) = 'negative'),
                    'neutral_count', COUNT(*) FILTER (WHERE lower(trim(fc.sentiment::text)) NOT IN ('positive','negative'))
                )
            )
            FROM (
                SELECT created_time, sentiment
                FROM filtered_comments
                GROUP BY DATE(created_time), created_time, sentiment
        ...
        'ads', (
            SELECT json_agg(
                json_build_object(
                    ...
                    'funnel', ba.funnel,
                    'total_comments', COALESCE(comment_counts.comment_count, 0)
                )
            )
            FROM brand_ads ba
            LEFT JOIN (
                ...
                FROM comments
                GROUP BY ad_id
            ) comment_counts ON ba.id = comment_counts.ad_id
        ),
        'comments', (
            SELECT json_agg(
                json_build_object(
...
CREATE INDEX IF NOT EXISTS idx_comments_ad_id_created_time ON public.comments(ad_id, created_time);
CREATE INDEX IF NOT EXISTS idx_comments_sentiment ON public.comments(sentiment);
CREATE INDEX IF NOT EXISTS idx_ads_funnel ON public.ads(funnel);
CREATE INDEX IF NOT EXISTS idx_ads_angle_type ON public.ads(angle_type);
