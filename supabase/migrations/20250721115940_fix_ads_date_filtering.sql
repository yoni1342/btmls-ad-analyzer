            ...
            SELECT json_build_object(
                ...
                'total_ads',     (
                    ...
                    WHERE (brand_id_param IS NULL OR a.brand_id = brand_id_param)
                      AND (funnel_param IS NULL OR funnel_param = 'all' OR a.funnel::text = funnel_param)
                      AND (angel_param IS NULL OR angel_param = 'all' OR a."Angel Type"::text = angel_param)
                      AND (start_date_ts IS NULL OR a."Created At"::timestamp with time zone >= start_date_ts)
                      AND (end_date_ts IS NULL OR a."Created At"::timestamp with time zone <= end_date_ts)
                )
            )
        ),
        'untracked_info', (
            SELECT json_build_object(
                'untracked_ads_count', (
                    ...
                    WHERE brand_id = brand_id_param
                      ...
                      AND (start_date_ts IS NULL OR "Created At"::timestamp with time zone >= start_date_ts)
                      AND (end_date_ts IS NULL OR "Created At"::timestamp with time zone <= end_date_ts)
                      AND (
                        "Angel" IS NULL
                        OR lower(trim("Angel")) = ''
