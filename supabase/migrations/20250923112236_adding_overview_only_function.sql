-- Create optimized function for brands page overview tab
-- This only returns aggregated data (counts, charts) without heavy individual records
...
BEGIN
    ...
    SELECT json_build_object(
        -- Sentiment chart data (monthly aggregation for better performance)
        'daily_sentiment_counts', COALESCE((
            SELECT json_agg(t)
            FROM (
                SELECT
                    CASE 
                        WHEN start_date_ts IS NULL AND end_date_ts IS NULL THEN
                            TO_CHAR(DATE_TRUNC('month', c.created_time), 'YYYY-MM-DD')::TEXT
                        ELSE
                            DATE(c.created_time)::TEXT 
                    END AS created_date,
                ...
                GROUP BY 
                    CASE 
                        WHEN start_date_ts IS NULL AND end_date_ts IS NULL THEN
                            DATE_TRUNC('month', c.created_time)
                        ELSE
                ...
                ORDER BY 
                    ...
                    END
                LIMIT CASE 
                    WHEN start_date_ts IS NULL AND end_date_ts IS NULL THEN 120 -- 10 years monthly
                    ELSE 365 -- 1 year daily
                END
            ) t
        ), '[]'::json),
        ...
        'untracked_info', json_build_object(
            ...
            -- No individual IDs for overview - just counts
