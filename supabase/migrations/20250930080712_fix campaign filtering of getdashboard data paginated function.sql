-- Fix campaign selection filtering issue in get_brands_tables_paginated function
-- When campaigns are selected/displayed, downstream tables (ad_sets, ads, comments) should only show data related to those campaigns
-- The issue was that related tables were not being filtered by the paginated_campaigns CTE
...
CREATE OR REPLACE FUNCTION get_brands_tables_paginated(
    ...
    -- Selection-based filtering parameters for interdependency
...
BEGIN
    ...
    IF primary_table_param = 'campaigns' THEN
        -- Paginated campaigns with ALL related data
        ...
        WHERE (brand_id_param IS NULL OR aa.brand_id = brand_id_param)
            ...
            -- FIX: Apply campaign selection filter if campaigns are explicitly selected
            AND (NOT has_campaign_selection OR camp.id::text = ANY(selected_campaign_ids_param));
        ...
        paginated_campaigns AS (
            SELECT * FROM base_campaigns
        ...
        related_ad_sets AS (
            ...
            -- FIX: When campaigns are selected via checkbox, filter by selected campaigns
            -- Otherwise, filter by paginated campaigns (all campaigns on current page)
            WHERE (
                    -- If campaigns are selected via checkbox, only show ad_sets for selected campaigns
                    (has_campaign_selection AND ads_set.campaign_id::text = ANY(selected_campaign_ids_param))
                    OR
                    -- If no campaign selection, show ad_sets for all paginated campaigns
                    (NOT has_campaign_selection AND ads_set.campaign_id IN (SELECT id FROM paginated_campaigns))
                )
                ...
                -- Additional selection-based filtering for ad sets
        ...
        related_ads AS (
            SELECT DISTINCT a.*
            FROM ads a
            -- FIX: Use related_ad_sets which is already filtered by paginated_campaigns
            WHERE a.ad_set_id IN (SELECT id FROM related_ad_sets)
                ...
                -- Additional selection-based filtering for ads
        ...
        related_comments AS (
            SELECT c.*
            FROM comments c
            -- FIX: Use related_ads which is already filtered by paginated_campaigns through related_ad_sets
            WHERE c.ad_id IN (SELECT id FROM related_ads)
                ...
                -- Additional comment-specific filtering
    ...
    ELSIF primary_table_param = 'adsets' THEN
        -- Similar logic for ad sets as primary table with selection-based filtering
        ...
        related_campaigns AS (
            SELECT DISTINCT camp.*
            FROM campaigns camp
            JOIN paginated_adsets pas ON camp.id = pas.campaign_id
        ),
        
        related_ads AS (
            SELECT DISTINCT a.*
            FROM ads a
            JOIN paginated_adsets pas ON a.ad_set_id = pas.id
            ...
            'ads', COALESCE((
                SELECT json_agg(
                    json_build_object(
                        ...
                        'ad_id', ra.id::text,
                        'ad_set_id', ra.ad_set_id::text,
                        'campaign_id', (SELECT campaign_id::text FROM paginated_adsets WHERE id = ra.ad_set_id LIMIT 1),
                        'Angel', ra.angle,
                        'Angel Type', ra.angle_type,
                        'angle_type', ra.angle_type,
                        'Explanation', ra.analysis_explanation,
    ...
    ELSIF primary_table_param = 'ads' THEN
        -- Similar logic for ads as primary table with selection-based filtering
        ...
        SELECT COUNT(*) INTO total_count FROM base_ads;
        ...
        related_campaigns AS (
            SELECT DISTINCT camp.*
            FROM campaigns camp
            JOIN ad_sets ads_set ON camp.id = ads_set.campaign_id
            ...
            'ads', COALESCE((
                SELECT json_agg(
                    json_build_object(
                        ...
                        'campaign_id', (SELECT campaign_id::text FROM related_adsets WHERE id = pa.ad_set_id LIMIT 1),
                        'Angel', pa.angle,
    ...
    ELSIF primary_table_param = 'comments' THEN
        -- Similar logic for comments as primary table with selection-based filtering
        ...
        SELECT COUNT(*) INTO total_count FROM base_comments;
        ...
        related_campaigns AS (
            SELECT DISTINCT camp.*
            FROM campaigns camp
            JOIN related_adsets ras ON camp.id = ras.campaign_id
        )
            ...
            'ads', COALESCE((
                SELECT json_agg(
                    json_build_object(
                        ...
                        'ad_id', ra.id::text,
                        'ad_set_id', ra.ad_set_id::text,
                        'campaign_id', (SELECT campaign_id::text FROM related_adsets WHERE id = ra.ad_set_id LIMIT 1),
                        'Angel', ra.angle,
                        'Angel Type', ra.angle_type,
                        'angle_type', ra.angle_type,
                        'Explanation', ra.analysis_explanation,
