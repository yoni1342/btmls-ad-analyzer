        ...
        'untracked_info', (
            SELECT json_build_object(
                'untracked_ads_count', (
                    ...
                    WHERE brand_id = brand_id_param AND (("Angel" IS NULL OR lower(trim("Angel")) = '' OR lower(trim("Angel")) = 'unknown') AND ("Angel Type" IS NULL OR lower(trim("Angel Type")) = 'unknown'))
                ),
                'untracked_comments_count', (
                    SELECT COUNT(*)
                    FROM comments c
                    LEFT JOIN ad_per_ad_account a ON c.ad_id = a.ad_id
                    WHERE a.brand_id = brand_id_param AND (c.sentiment IS NULL OR lower(trim(c.sentiment)) = '' OR lower(trim(c.sentiment)) = 'unknown')
                ),
                'untracked_ad_ids', (
