-- Migration: trim whitespace in sentiment comparison
...
COUNT(*) FILTER (WHERE lower(trim(c.sentiment)) = 'positive') AS positive_count,
COUNT(*) FILTER (WHERE lower(trim(c.sentiment)) = 'negative') AS negative_count,
COUNT(*) FILTER (WHERE lower(trim(c.sentiment)) NOT IN ('positive', 'negative')) AS neutral_count
FROM comments c
