-- Fix GROUP BY error in get_dashboard_data function
-- The error occurs in the comments section where we're trying to use lc.created_at without proper grouping
...
BEGIN
                ...
                'ads', COALESCE((
                    SELECT json_agg(
                        json_build_object(
                            ...
                            'total_comments', (
                                SELECT COUNT(*)
                                FROM lifetime_comments lc_count
                                WHERE lc_count.ad_id = la.id
                            )
                        )
                    )
                    FROM lifetime_ads la
                ...
                'comments', COALESCE((
                    SELECT json_agg(
                        json_build_object(
                            ...
                            'brand', lc.brand,
                            'ad_title', (SELECT title FROM lifetime_ads la_title WHERE la_title.id = lc.ad_id LIMIT 1),
                            'Angel Type', (SELECT angle_type FROM lifetime_ads la_angle WHERE la_angle.id = lc.ad_id LIMIT 1),
                            'meta_cluster', (SELECT meta_cluster FROM comment_cluster cc WHERE cc.comment_id = lc.comment_id LIMIT 1),
                            'funnel', (SELECT funnel FROM lifetime_ads la_funnel WHERE la_funnel.id = lc.ad_id LIMIT 1)
                        )
                    )
    ...
    -- Original logic for filtered queries (keeping the same structure as your latest migration)
