-- [`supabase/migrations/20250707120000_update_getdashboarddata_include_meta_cluster_and_angel_type.sql`](supabase/migrations/20250707120000_update_getdashboarddata_include_meta_cluster_and_angel_type.sql:1)

CREATE OR REPLACE FUNCTION public.get_dashboard_data(
    brand_id_param BIGINT,
    start_date_param TIMESTAMPTZ,
    end_date_param TIMESTAMPTZ,
    sentiment_param TEXT
)
RETURNS JSON
AS $$
DECLARE
...
BEGIN
    WITH filtered_comments AS (
        ...
        WHERE
            ...
            AND (sentiment_param IS NULL 
                 OR sentiment_param = 'all' 
                 OR lower(trim(c.sentiment)) = lower(sentiment_param))
    )
        ...
        'daily_sentiment_counts', (
            ...
            FROM (
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE lower(trim(fc.sentiment)) NOT IN ('positive','negative'))
                                                                                 AS neutral_count
            ...
            SELECT json_build_object(
                'untracked_ads_count', (
                    ...
                    WHERE brand_id = brand_id_param
                      AND (
                          ("Angel" IS NULL OR lower(trim("Angel")) = '' OR lower(trim("Angel")) = 'unknown')
                          AND ("Angel Type" IS NULL OR lower(trim("Angel Type")) = 'unknown')
                      )
                ),
                'untracked_comments_count', (
                    ...
                    WHERE a.brand_id = brand_id_param
                      AND (c.sentiment IS NULL OR lower(trim(c.sentiment)) = '' OR lower(trim(c.sentiment)) = 'unknown')
                ),
                'untracked_ad_ids', (
        ...
        'comments', (
            SELECT json_agg(
                json_build_object(
                    ...
                    'theme',        c.theme,
                    'meta_cluster', cl.meta_cluster,
                    'Angel Type',   a."Angel Type"
                )
            )
            FROM comments c
            LEFT JOIN ad_per_ad_account a ON c.ad_id = a.ad_id
            LEFT JOIN comment_cluster cl ON c.comment_id = cl.comment_id
            WHERE (brand_id_param IS NULL OR a.brand_id = brand_id_param)
              AND (start_date_param IS NULL OR c.created_time >= start_date_param)
              AND (end_date_param IS NULL OR c.created_time <= end_date_param)
              AND (sentiment_param IS NULL 
