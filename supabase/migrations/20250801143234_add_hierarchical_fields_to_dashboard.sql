-- Add hierarchical relationship fields to dashboard data for proper filtering
-- This enables campaign -> ad set -> ads filtering functionality
...
BEGIN
    ...
    WITH filtered_comments AS (
        ...
        WHERE
            ...
            AND (funnel_param IS NULL OR funnel_param = 'all' OR a.funnel_stage::text = funnel_param)
    ...
    SELECT json_build_object(
        ...
        'campaigns', (
            SELECT json_agg(
                json_build_object(
                    ...
                    'account_id', camp.account_id::text,
                    'account_name', aa.ad_name,
        ...
        'ad_sets', (
            SELECT json_agg(
                json_build_object(
                    'ad_set_id', ads_set.id::text,
                    'ad_set_name', ads_set.name,
                    'campaign_id', camp.id::text,
                    'campaign_name', camp.name,
                    'status', ads_set.status,
                    'effective_status', ads_set.effective_status,
                    'optimization_goal', ads_set.optimization_goal,
                    'bid_strategy', ads_set.bid_strategy,
                    'daily_budget', ads_set.daily_budget,
                    'lifetime_budget', ads_set.lifetime_budget,
                    'budget_remaining', ads_set.budget_remaining,
                    'start_time', ads_set.start_time,
                    'end_time', ads_set.end_time,
                    'created_time', ads_set.created_time,
                    'lifetime_imps', ads_set.lifetime_imps,
                    'destination_type', ads_set.destination_type
                )
            )
...
-- Add comment for the enhancement
COMMENT ON FUNCTION get_dashboard_data(integer, text, text, text, text, text) 
IS 'Enhanced dashboard function with hierarchical campaign -> ad set -> ads relationships for filtering support';
