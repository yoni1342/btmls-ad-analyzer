-- Final fix for GROUP BY error - the issue is in the comments aggregation section
-- We need to avoid mixing non-aggregated columns with GROUP BY clauses
       ...
       (adset_optimization_param IS NULL OR adset_optimization_param = 'all') THEN
        ...
        IF return_full_data THEN
            ...
            SELECT json_build_object(
                ...
                'comments', COALESCE((
                    SELECT json_agg(sub.comment_json)
                    FROM (
                        SELECT json_build_object(
                            'id', lc.id,
