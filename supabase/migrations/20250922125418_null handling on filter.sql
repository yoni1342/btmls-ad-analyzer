-- Fix handling of 'Unknown' angel filter in get_dashboard_data function
-- When angel_param is 'Unknown', it should match records where angle_type IS NULL

-- This is a simple update to add the Unknown filter logic to the existing function
-- We only need to update the filtered_ads CTE part where angel filtering happens

-- Update the get_dashboard_data function to handle 'Unknown' angel filter
...
BEGIN
    ...
    -- Determine if we should return full data or just aggregates
    return_full_data := (
        has_date_filter OR 
        (sentiment_param IS NOT NULL AND sentiment_param != 'all') OR 
        (funnel_param IS NOT NULL AND funnel_param != 'all') OR 
        (angel_param IS NOT NULL AND angel_param != 'all')
    );
    ...
    IF NOT has_date_filter AND 
       ...
       (angel_param IS NULL OR angel_param = 'all') THEN
        ...
        -- Check if we should return full data (Dashboard Page) or just aggregates (Report Page)
        IF return_full_data THEN
            -- Return full data for Dashboard Page
            WITH lifetime_ads AS (
                SELECT DISTINCT a.*
                FROM ads a
            ...
            SELECT json_build_object(
                'daily_sentiment_counts', COALESCE((
                    ...
                    FROM (
                        SELECT
                            TO_CHAR(DATE_TRUNC('month', lc.created_time), 'YYYY-MM-DD')::TEXT AS created_date,
                            COUNT(*) FILTER (WHERE lower(trim(lc.sentiment::text)) = 'positive') AS positive_count,
                            COUNT(*) FILTER (WHERE lower(trim(lc.sentiment::text)) = 'negative') AS negative_count,
                            COUNT(*) FILTER (WHERE lower(trim(lc.sentiment::text)) NOT IN ('positive','negative')) AS neutral_count
                        FROM lifetime_comments lc
                        WHERE lc.created_time IS NOT NULL
                        GROUP BY DATE_TRUNC('month', lc.created_time)
                        ORDER BY DATE_TRUNC('month', lc.created_time)
                        LIMIT 120
                    ) t
                ), '[]'::json),
                ...
                'funnel_distribution', COALESCE((
                    ...
                    FROM (
                        SELECT 
                            COALESCE(la.funnel::text, 'Unprocessed') AS name, 
                            COUNT(*) AS count
                        FROM lifetime_ads la
                        GROUP BY COALESCE(la.funnel::text, 'Unprocessed')
                        ORDER BY 
                            CASE COALESCE(la.funnel::text, 'Unprocessed')
                                WHEN 'TOF' THEN 1
                                WHEN 'MOF' THEN 2
                                WHEN 'BOF' THEN 3
                ...
                'comments', COALESCE((
                    SELECT json_agg(
                        json_build_object(
                            ...
                            'message', lc.message,
                            'created_time', lc.created_time,
                            'ad_id', lc.ad_id::text,
                            'created_at', lc.created_at,
                            'theme', lc.theme,
                            'sentiment', lc.sentiment,
                            'brand', lc.brand,
                            'ad_title', (SELECT title FROM lifetime_ads WHERE id = lc.ad_id LIMIT 1),
                            'Angel Type', (SELECT angle_type FROM lifetime_ads WHERE id = lc.ad_id LIMIT 1),
                            'meta_cluster', (SELECT meta_cluster FROM comment_cluster WHERE comment_id = lc.comment_id LIMIT 1),
                            'funnel', (SELECT funnel FROM lifetime_ads WHERE id = lc.ad_id LIMIT 1)
                        )
                    )
                    FROM lifetime_comments lc
                    LIMIT 5000
    ...
    -- IMPORTANT: This is where we fix the Unknown angel filter
    WITH filtered_ads AS (
        ...
        WHERE
            ...
            AND (
                ...
                OR angel_param = 'all' 
                OR (
                    -- Special handling for 'Unknown' - match NULL angle_type
                    angel_param = 'Unknown' AND a.angle_type IS NULL
                )
                OR (
                    -- Normal handling for other angel types
                    angel_param != 'Unknown' AND a.angle_type::text = angel_param
                )
            )
    ),
    ...
    SELECT json_build_object(
        ...
        'funnel_distribution', COALESCE((
            ...
            FROM (
                ...
                ORDER BY 
                    CASE COALESCE(funnel::text, 'Unprocessed')
                        WHEN 'TOF' THEN 1
                        WHEN 'MOF' THEN 2
                        WHEN 'BOF' THEN 3
                        WHEN 'Unprocessed' THEN 4
