...
setAd(data.ad);
const commentsWithAngle = data.comments.map((comment: Comment) => ({
...comment,
...
<img src={ad.image} alt="Ad creative" className="rounded-lg w-full h-auto object-contain max-h-96" />
</div>
)}
{ad.video_url && (
<div className="flex-1 min-w-[300px] max-w-md">
<video src={ad.video_url} controls className="rounded-lg w-full h-auto object-contain max-h-96" />
</div>
)}
</div>
                {ad.post_link && <a href={ad.post_link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mt-4 inline-block">View Post</a>}
            </div>
